...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isWall)
        {
            isWallJump = false;
            //벽점프상태
            pRig2D.linearVelocity = new Vector2(pRig2D.linearVelocityX, pRig2D.linearVelocityY);
            //벽을 잡고있는 상태에서 점프
            if (Input.GetKeyDown(KeyCode.W))
            {
                isWallJump = true;
    ...
    {
        isWallJump = false;
        //pRig2D.constraints = RigidbodyConstraints2D.FreezePositionX;
    }
    ...
    {
        Instantiate(dust, transform.position + new Vector3(-0.1f, -0.5f, 0), Quaternion.identity);
    }
